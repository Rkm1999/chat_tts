...
class DecoderProxy:
    ...
    def __init__(self, onnx_path: str, onnx_provider: str = 'CPU', chunk_size: int = 12):
        ...
        self._speaker_volume: float = 1.0
            ...
            if self.streaming_results.get(task_id):
                ...
                if audio_to_play is not None and len(audio_to_play) > 0:
                    if self._speaker_muted:
                        spk_audio = np.zeros_like(audio_to_play)
                    elif self._speaker_volume != 1.0:
                        spk_audio = np.clip(audio_to_play * self._speaker_volume, -1.0, 1.0)
                    else:
                        spk_audio = audio_to_play
                    self.play_q.put(SpeakerRequest(msg_type="AUDIO", audio=spk_audio))
                    if self._chunk_callback is not None:
                        self._chunk_callback(audio_to_play)  # mic gets raw audio; mic has its own volume
